...
export default function CatSprite({ position, color, storeText }) {
  const transform = `translate(${position.x}px, ${position.y}px) rotate(${position.rotation}deg)`;
  return (
    <div style={{ textAlign: 'center', position: 'relative', marginTop:"30px", marginLeft:'20px' }}>
      <div style={{
        position: 'absolute',
        left: `${position.x}px`,
        top: `${position.y}px`,
        transform: `translate(-50%, -100%)`  // Center the text above the SVG
      }}>
        {storeText}
      </div>
      <svg
